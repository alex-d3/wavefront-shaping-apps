...
namespace ProtoBuf_test
{
    class Program
    {
        ...
        {
            FastArray<Complex> cf = new FastArray<Complex>(1);

            //RuntimeTypeModel.Default.Add(typeof(Complex*), false).SetSurrogate(typeof(PointerSurrogate));
    ...
    unsafe class Test
    {
        ...
        {
            using (var outfile = File.Create("d:/test.proto"))
            {
        ...
        {
            using (var outfile = File.OpenRead("d:/test.proto"))
            {
                Serializer.Deserialize<Test>(outfile);
            }
        }
        ...
        public Test()
        {
            ...
            ptr = (Complex*)Alloc(1024);
            ptr[0] = new Complex(1.0, 5.0);
        }
    ...
    [ProtoContract]
    unsafe public class PointerSurrogate
    {
        [ProtoMember(1)]
        public byte[] Value { get; set; }

        public static implicit operator Complex*(PointerSurrogate surrogate)
        {
            return surrogate == null ? null : (Complex*)Alloc(surrogate.Value.Length);
        }

        public static implicit operator PointerSurrogate(Complex* source)
        {
            if (source == null)
                return null;

            PointerSurrogate surrogate = new PointerSurrogate();
            surrogate.Value = new byte[SizeOf(source)];

            Marshal.Copy(new IntPtr(source), surrogate.Value, 0, surrogate.Value.Length);
